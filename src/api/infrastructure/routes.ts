...
router.post(
  "/documents/",
  OneRequestsPerMinuteLimiter,
  TenRequestsPerDayLimiter,
...
router.get("/stats/:id", TenRequestsPerDayLimiter, OneHundredRequestsPerDayLimiter, async (req, res) => {
  const events = await fetch(
    `https://api.applicationinsights.io/v1/apps/${config.get(
      "Azure.ApplicationInsightAppID"
    )}/events/requests?timespan=P3Y6M4DT12H30M5S&$search=/api/documents/${req.params.id}`,
    {
      headers: {
    ...
    .then((data) => data.value.map((v: { client: any; timestamp: any }) => ({ ...v.client, timestamp: v.timestamp })))
