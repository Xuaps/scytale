...
describe("Upload file", () => {
  it("should upload the file", async () => {
    ...
    const { result, waitFor } = renderHook(() =>
      useUploadFile(createFileUploadedDoc, uploadFileMock, setState)
    );
  ...
  it("should move the file to the list of uploaded files", async () => {
    ...
    expect(setState).toHaveBeenCalledWith({
      upload: {
        uploadedFiles: [{ id: "anyId", name: "", password: "" }],
        encryptedFiles: [],
      },
    });
  });
  it("should persist the changes into the local storage", async () => {
    ...
    expect(localStorage.getItem("files")).toEqual(
      JSON.stringify([{ id: "anyId", name: "", password: "" }])
    );
  });
});
